...
async function refreshCallHistory() {
  ...
  console.log("CALL HISTORY STARTED");
  ...
  try {
    ...
    console.log("CALL HISTORY BEFORE API");

    const result = await api("/api/calls/history");

    console.log("CALL HISTORY AFTER API", result);
    ...
    result.calls.forEach((call) => {
      ...
      row.innerHTML = `
        <td>${call.startedAt ? new Date(call.startedAt).toLocaleTimeString() : "-"}</td>
        ...
        <td class="call-history-action">

          <button
            class="secondary"
            data-open-lead="${call.leadId}">
            Open Lead
          </button>

          <button
            class="primary"
            data-redial="${call.leadId}">
            Call Again
          </button>
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="7"
            style="text-align:center;padding:25px;color:#ff7777;">
            ${error.message}
        </td>
      </tr>
    `;
